...
export default class Drawflow {
    ...
    noderegister:any = {};
    render:any;
    drawflow:any = { "drawflow": { "Home": { "data": {} }}};
    ...
    constructor(container: ElementRef, renderer: Renderer2,render?: object, parent?: object) {
    ...
    start () {
        ...
        this.precanvas = this.renderer.createElement('div');
        this.precanvas.classList.add("drawflow");
        this.renderer.appendChild(this.container,this.precanvas);
    ...
    position(e:any) {
      ...
      if(this.drag_point) {
        ...
        const searchConnection = this.drawflow.drawflow[this.module].data[nodeId].outputs[output_class].connections.findIndex(function(item:any,i:any) {
    ...
    dragEnd(e:any) {
      if (e.type === "touchend") {
          ...
          console.log("DRAG END",e,e_pos_x,e_pos_y);
          var ele_last = e.target;
      } else {
          var e_pos_x:number = e.clientX;
          var e_pos_y:number = e.clientY;
          var ele_last = e.target;
      }
      ...
      if(this.connection === true) {
        if(ele_last.classList[0] === 'input' || (this.force_first_input && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node'))) {
          ...
          if(this.force_first_input && (ele_last.closest(".drawflow_content_node") != null || ele_last.classList[0] === 'drawflow-node')) {
           ...
           var input_class:string = ele_last.classList[1];
         }
    ...
    contextmenu(e:any) {
        ...
        if(this.node_selected || this.connection_selected) {
            var deletebox = this.renderer.createElement('div');
            ...
            this.container.appendChild(this.precanvas,deletebox);
    ...
    zoom_enter(event:any, delta:any = null) {
      if (event.ctrlKey) {
    ...
    createCurvature(start_pos_x:number, start_pos_y:number, end_pos_x:number, end_pos_y:number, curvature_value:number, type:string) {
    ...
    drawConnection(ele:any) {
      var connection = this.renderer.createElement("svg",'http://www.w3.org/2000/svg');
      this.connection_ele = connection;
      var path = this.renderer.createElement("path",'http://www.w3.org/2000/svg');
    ...
    addConnection(id_output:string, id_input:string, output_class:string, input_class:string) {
      ...
      if(nodeOneModule === nodeTwoModule) {
        ...
        if(exist === false) {
          ...
          //Draw connection
            ...
            connection.appendChild(path);
            this.container.appendChild(this.precanvas,connection);
            this.updateConnectionNodes('node-'+id_output);
            this.updateConnectionNodes('node-'+id_input);
          }
    ...
    updateConnectionNodes(id:any) {
      ...
      Object.keys(elemsOut).map(function(item:any, index:any) {
        if(elemsOut[item].querySelector('.point') === null) {
          ...
          const points:any = elemsOut[item].querySelectorAll('.point');
          let linecurve = '';
          ...
          if(reroute_fix_curvature) {
            reoute_fix.forEach((itempath:any, i:any) => {
      ...
      Object.keys(elems).map(function(item:any, index:any) {
        ...
        if(elems[item].querySelector('.point') === null) {
          ...
          const points:any = elems[item].querySelectorAll('.point');
          let linecurve = '';
          const reoute_fix:any = [];
          if(Array.isArray(points)){
            points.forEach((item:any, i:any) => {
    ...
    createReroutePoint(ele:any) {
        ...
        this.connection_selected = null;
        const point = this.renderer.createElement("circle",'http://www.w3.org/2000/svg');
    ...
    addNode (name:any, num_in:any, num_out:any, ele_pos_x:any, ele_pos_y:any, classoverride:any, data:any, html:any, typenode = false) {
      if (this.useuuid) {
        ...
        var newNodeId:any = this.nodeId.toString();
      }
      ...
      const inputs = this.renderer.createElement('div');
      inputs.classList.add("inputs");
  
      const outputs = this.renderer.createElement('div');
      ...
      for(var x = 0; x < num_out; x++) {
        const output = this.renderer.createElement('div');
    ...
    addNodeImport (dataNode:any, precanvas:any) {
      ...
      Object.keys(dataNode.inputs).map(function(input_item, index) {
        const input = renderer.createElement('div');
        input.classList.add("input");
        input.classList.add(input_item);
        inputs.appendChild(input);
        Object.keys(dataNode.inputs[input_item].connections).map(function(output_item, index) {
  
          // var connection = renderer.createElementNS('http://www.w3.org/2000/svg',"svg");
          // var path = renderer.createElementNS('http://www.w3.org/2000/svg',"path");
          var connection = renderer.createElement("svg",'http://www.w3.org/2000/svg');
          var path = renderer.createElement("path",'http://www.w3.org/2000/svg');
          path.classList.add("main-path");
          path.setAttributeNS(null, 'd', '');
          // path.innerHTML = 'a';
          connection.classList.add("connection");
      ...
      if(dataNode.typenode === false) {
        if(dataNode.html.type === 'string'){
          content.innerHTML = dataNode.html.data;
        }else if(dataNode.html.type === 'object'){
          if(dataNode.html.data.title){
            let title = this.renderer.createElement('div');
            title.classList.add("title-box");
            title.innerHTML = dataNode.html.data.title;
            content.appendChild(title);
          }
          if(dataNode.html.data.box){
            let box = this.renderer.createElement('div');
            box.classList.add("box");
            box.innerHTML = dataNode.html.data.box;
            content.appendChild(box);
          }
        }
        ...
        Object.keys(dataNode.outputs[output_item].connections).map(function(input_item, index) {
          ...
          if(points !== undefined) {
            ...
            points.forEach((item:any, i:any) => {
              ...
              const point = renderer.createElement("circle",'http://www.w3.org/2000/svg');
              point.classList.add("point");
    ...
    updateNodeDataFromId(id:any, data:any) {
      ...
      if(this.module === moduleName) {
        ...
        Object.entries(data).forEach(function (key:any, value:any) {
          if(typeof key[1] === "object") {
            insertObjectkeys(null, key[0], key[0]);
          } else {
            var elems:any = content.querySelectorAll('[df-'+key[0]+']');
    ...
    removeNodeInput(id:any, input_class:any) {
      ...
      let nodeUpdates:any = [];
      connections.forEach((item:any, i:any) => {
    ...
    removeConnection() {
      if(this.connection_selected != null) {
        ...
        var index_out = this.drawflow.drawflow[this.module].data[listclass[2].slice(14)].outputs[listclass[3]].connections.findIndex(function(item:any,i:any) {
        ...
        var index_in = this.drawflow.drawflow[this.module].data[listclass[1].slice(13)].inputs[listclass[4]].connections.findIndex(function(item:any,i:any) {
